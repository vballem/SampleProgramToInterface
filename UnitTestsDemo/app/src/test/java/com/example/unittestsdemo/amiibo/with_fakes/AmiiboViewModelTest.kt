...
class AmiiboViewModelTest {
    ...
    private lateinit var fakeGetAmiibosUseCase: FakeGetAmiibosUseCase

    @Before
    fun setUp() {
        Dispatchers.setMain(testDispatcher)
        fakeGetAmiibosUseCase = FakeGetAmiibosUseCase()
        viewModel = AmiiboViewModel(fakeGetAmiibosUseCase)
    }
    ...
    fun `fetchAmiibos updates state to Success when use case returns data`() = testScope.runTest {
        ...
        fakeGetAmiibosUseCase.setAmiiboList(amiiboList)
    ...
    fun `fetchAmiibos updates state to Error when use case throws exception`() = testScope.runTest {
        // Given
        fakeGetAmiibosUseCase.setShouldThrowError(true)
